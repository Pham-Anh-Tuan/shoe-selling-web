...
const ProductList = () => {
    ...
    const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>, id: number) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                setRows((prevRows) =>
                    prevRows.map((row) =>
                        row.id === id ? { ...row, image: reader.result as string } : row
                    )
                );
            };
    ...
    // const [sizeAmounts, setSizeAmounts] = useState<
    //     { id: number; size: number; amount: number }[]
    // >();
    // [{ id: 1, size: 38, amount: 13 }]

    const [rows, setRows] = useState<
        { id: number; color: string; image: string | null; sizeAmounts: { id: number; size: number; amount: number }[] }[]
    >([{ id: 1, color: "#1e3a8a", image: Img1, sizeAmounts: [{ id: 1, size: 38, amount: 13 }] }]);
    ...
    //         if (button.id === 'dropdownDefaultButton') {
    //             dropdownId = button.getAttribute('data-dropdown-toggle');
    //         }

    //         const dropdownMenu = dropdownId ? document.getElementById(dropdownId) : null;

    //         if (dropdownMenu) {
    //             // Kiểm tra nếu dropdown đã được khởi tạo, tránh lỗi lặp
    //             if (!dropdownMenu.dataset.initialized) {
    //                 const dropdownInstance = new Dropdown(dropdownMenu);
    //                 dropdownMenu.dataset.initialized = "true"; // Đánh dấu đã khởi tạo
    //             }
    //         }
    //     });
    // }, [rows]);
    ...
    const addRow = () => {
        ...
        setRows([...rows, { id: temp, color: "#ffffff", image: null, sizeAmounts: [{ id: temp + 101, size: 0, amount: 0 }] }]);
    };
    ...
    const addSizeAmount = (rowId: number, newSizeAmount: { id: number; size: number; amount: number }) => {
        newSizeAmount.id = Date.now() % 1000000;
        setRows(prevRows =>
            prevRows.map(row =>
                row.id === rowId
                    ? { ...row, sizeAmounts: [...row.sizeAmounts, newSizeAmount] }
                    : row
            )
        );
    };

    const deleteSizeAmount = (rowId: number, sizeAmountId: number) => {
        setRows(prevRows =>
            prevRows.map(row =>
                row.id === rowId
                    ? {
                        ...row,
                        sizeAmounts: row.sizeAmounts.filter(sizeAmount => sizeAmount.id !== sizeAmountId)
                    }
                    : row
            )
        );
                                ...
                                <tbody>
                                    ...
                                    <tr className="border-b dark:border-gray-700">
                                        ...
                                        <td className="px-4 py-3 flex items-center justify-end">
                                            ...
                                            <div id="giay-the-thao-2-dropdown" className="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul className="py-1 text-sm" aria-labelledby="giay-the-thao-2-dropdown-button">
                                                    <li>
                                                        <button type="button" data-modal-target="updateProductModal" data-modal-toggle="updateProductModal" className="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                            <svg className="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                                <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                        ...
                        <form action="#">
                            <div className="grid gap-4 mb-4 sm:grid-cols-2">
                                ...
                                {/* <div>
                                    ...
                                    <input type="text" name="brand" id="brand" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="Product brand" required />
                                </div> */}
                                <div>
                                    <label htmlFor="price" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Giá</label>
                                    <p className="block w-full"><input type="number" name="price" id="price" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="2,200,000" required />₫</p>
                                </div>
                                <div>
                            ...
                            <div className="overflow-x-auto mb-4">
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                        <tr>
                                            <th scope="col" className="px-4 py-4">Màu</th>
                                    ...
                                    <tbody>
                                        <tr className="border-b dark:border-gray-700">
                                            <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><div className="h-4 w-4 rounded-full inline-block mr-2 bg-blue-900 border border-gray-200"></div> </th>
                                            <td className="px-4 py-3">
                                                <div className="flex items-center mr-3">
                                                    <img src={Img1} alt="iMac Front Image" className="h-14 w-auto mr-3" />
                                                </div>
                                            </td>
                                            <td className="px-4 py-3">
                                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                                        <tr>
                                                            <th scope="col" className="px-4 py-4">Kích thước</th>
                    ...
                    <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <form action="#">
                            <div className="grid gap-4 mb-4 sm:grid-cols-2">
                                <div>
                                    <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tên</label>
                                    <input type="text" name="name" id="name" value="Giày Thể Thao Sneaker MULGATI YC25051P" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="Ex. Apple iMac 27&ldquo;" />
                                </div>
                                <div>
                                    <label htmlFor="price" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Giá</label>
                                    {/* <p className="block w-full"><input type="number" value="2200000" name="price" id="price" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="" />đ</p> */}
                                    <p className="block w-full"><input type="number" value="2200000" name="price" id="price" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="2,200,000" required />₫</p>
                                </div>
                                <div>
                                    <label htmlFor="category" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Loại</label>
                                    <select id="category" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500">
                                ...
                                <div className="sm:col-span-2">
                                    ...
                                    <textarea id="description" rows={5} className="overflow-auto whitespace-pre-wrap block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-orange-500 focus:border-orange-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="Viết chi tiết cho phần mô tả sản phẩm"
                                        defaultValue={`Chất liệu cao cấp: Giày được làm từ da bò thật, mềm mại, bền bỉ, giúp ôm chân thoải mái và thoáng khí. \nThiết kế năng động: Phối màu trắng kem sang trọng với điểm nhấn sọc đen tạo phong cách trẻ trung, dễ dàng phối đồ.\nĐế cao su đúc nguyên khối: Nhẹ, êm chân, hỗ trợ di chuyển linh hoạt và chống trơn trượt hiệu quả.\nGia công tỉ mỉ: Đường may chắc chắn, hoàn thiện tinh tế, đảm bảo độ bền lâu dài.\nỨng dụng linh hoạt: Phù hợp cho nhiều dịp: đi chơi, dạo phố, đi làm, du lịch,...`}>
                                    </textarea>
                                </div>
                            </div>

                            <div className="flex items-center space-x-4 mb-1">
                                <button type="button" onClick={addRow} className="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg px-1 py-1 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800 text-xs">Thêm màu</button>
                            </div>
                                    ...
                                    <tbody>
                                        {rows.map((row) => (
                                            <tr key={row.id} className="border-b dark:border-gray-700">
                                                {/* <th scope="row" className="flex flex-col items-center justify-center space-y-1 px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    <input type="color" defaultValue={row.color} className="p-1 h-10 w-14 block bg-white cursor-pointer rounded-lg disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700" id="hs-color-input" title="Choose your color" />
                                                </th> */}
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex flex-col items-center justify-center space-y-1">
                                                        <label className="cursor-pointer bg-orange-500 text-white px-1 py-1 rounded-lg hover:bg-orange-600 text-xs">
                                                            Chọn ảnh
                                                            <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, row.id)} />
                                                        </label>

                                                        <div className="mt-1 border w-fit">
                                                            <img src={row.image} alt="Uploaded" className="w-16 h-16 object-cover rounded-lg shadow" />
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="flex flex-col items-center justify-center space-y-1">
                                                        <table className="table-auto">
                                                            ...
                                                            <tbody>
                                                                {row.sizeAmounts.map((sizeAmount) => (
                                                                    <tr>
                                                                        <td>
                                                                            <input type="number" defaultValue={sizeAmount.size} name="size" id="" className="w-12 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="2,200,000" required />
                                                                            -
                                                                        </td>
                                                                        <td>
                                                                            <input type="number" defaultValue={sizeAmount.amount} name="amount" id="" className="w-12 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-600 focus:border-orange-600 p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500" placeholder="2,200,000" required />
                                                                        </td>
                                                                        <td>
                                                                            <button onClick={() => deleteSizeAmount(row.id, sizeAmount.id)} type="button" className="text-gray-400 bg-transparent hover:text-red-600 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:text-red-600">
                                                                                <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                                                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                                                                                </svg>
                                                                                <span className="sr-only">Close modal</span>
                                                ...
                                                <td className="px-4 py-3 flex flex-col items-center justify-end">
                                                    <div className='flex flex-col items-center justify-center space-y-2'>
                                                        <button
                                                            type='button'
                                                            onClick={() => addSizeAmount(row.id, { id: 0, size: 0, amount: 0 })}
                    ...
                    <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex items-center space-x-3 sm:space-x-4">
                                <button type="button" className="text-white inline-flex items-center bg-orange-700 hover:bg-orange-800 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-orange-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800">
                ...
                <div className="relative p-4 w-full max-w-md max-h-full">
                    <div className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div className="flex justify-center items-center space-x-4">
                            <button data-modal-toggle="deleteModal" type="button" className="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-orange-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
