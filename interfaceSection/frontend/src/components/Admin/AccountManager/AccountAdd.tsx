...
export const AccountAdd: React.FC<AccountAddProps> = ({ toggleAdd }) => {
    interface Account {
        email: string;
        fullName: string;
        password: string;
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (account.imageFile) {
            formData.append("imageName", account.imageFile.name);
        ...
        <div className="relative p-4 w-full max-w-2xl max-h-full">
            <div className="relative p-4 bg-white rounded-md shadow dark:bg-gray-800 sm:p-5">
                <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Tạo tài khoản</h3>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mật khẩu <span className="text-red-500 text-xs">*</span></label>
                        ...
                        <div>
                            <label htmlFor="confirm-password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white" >Nhập lại mật khẩu <span className="text-red-500 text-xs">*</span></label>
                            <div className="relative block w-full">
                                <input
                    ...
                    <div className="flex flex-col items-center justify-center space-y-1">
                        <label htmlFor="title" className="block mb-2 text-base font-medium text-gray-900 dark:text-white">Ảnh Đại Diện <span className="text-red-500 text-xs">*</span></label>
                        <div className="mb-1">
                            <label className="">
                    ...
                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input onChange={(e) => setShippingAddress(e.target.value)}
                    ...
                    <div className="mb-4 flex items-center gap-4">
                        ...
                        <div className="flex gap-4 w-full dark:text-red-500">
                            <label><input type="radio" name="gender" value={1} onChange={() => setGender(1)} required /> Nam</label>
                            <label><input type="radio" name="gender" value={2} onChange={() => setGender(2)} required /> Nữ</label>
                            <label><input type="radio" name="gender" value={3} onChange={() => setGender(3)} required /> Khác</label>
                        </div>
                    </div>
                    ...
                    <div className="mb-4 flex items-center gap-4">
                        ...
                        <div className="flex gap-4 w-full dark:text-red-500">
                            <label><input type="radio" name="role" value={1} onChange={() => setRole(2)} required /> Khách hàng</label>
                            <label><input type="radio" name="role" value={2} onChange={() => setRole(3)} required /> Nhân viên</label>
                            <label><input type="radio" name="role" value={3} onChange={() => setRole(1)} required /> Quản trị viên</label>
                        </div>
                    </div>
                    ...
                    <div className="pb-4 flex items-center gap-4 border-b">
                        ...
                        <div className="flex gap-4 w-full dark:text-red-500">
                            <label><input type="radio" name="status" value={1} onChange={() => setStatus(1)} required /> Active</label>
                            <label><input type="radio" name="status" value={0} onChange={() => setStatus(0)} required /> Inactive</label>
                        </div>
                    </div>
